...
<html>
<head>
    <title>分页查询</title><link rel="stylesheet" href="resources/bootstrap-3.3.7-dist/css/bootstrap.min.css">
...
<div style="width: 80%;height:100vh;margin-left:10% ">
    ...
    <nav aria-label="Page navigation" style="margin-left: 5%">
        <ul class="pagination">
            <li>
                <a href="/selectWeaponForLike?currentPage=${currentPage-1}&weaponname=${weaponname}" aria-label="Previous">
                    <span aria-hidden="true">&laquo;</span>
                </a>
            </li>
            <c:forEach items="${array}"  varStatus="i">
                <li><a href="/selectWeaponForLike?currentPage=${i.count}&weaponname=${weaponname}">${i.count}</a></li><!--获取集合索引值-->
            </c:forEach>
            <li>
                <a href="/selectWeaponForLike?currentPage=${currentPage+1}&weaponname=${weaponname}" aria-label="Next" >
...
window.location.href="/likeupdata?weaponid="+weaponid+"&weaponname="+weaponname+"&launchplace="+launchplace+"&range="+range+"&amount="+amount+"&currentPage="+${currentPage};
});
</script>
</html>
