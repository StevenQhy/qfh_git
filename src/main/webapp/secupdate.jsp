    ...
    <div class="main_container">
        ...
        <div class="right_col" role="main">
            <div class="container" style="width: 400px;">
                ...
                <form action="${pageContext.request.contextPath}/updateSecUserServlet" method="post">

                    <input type="hidden" name="id" value="${secuser.id}">
                    <div class="form-group">
                        <label for="name">姓名：</label>
                        <input value="${secuser.name}" type="text" class="form-control" id="name" name="name" placeholder="请输入姓名">
                    </div>

                    <div class="form-group">
                        <label for="address">籍贯：</label>
                        <select name="address" class="form-control" id="address">
                            <c:if test="${secuser.address == '广东'}">
                                <option value="广东" selected>广东</option>
                                <option value="广西">广西</option>
                                <option value="湖南">湖南</option>
                            </c:if>
                            <c:if test="${secuser.address == '广西'}">
                                <option value="广东" >广东</option>
                                <option value="广西" selected>广西</option>
                                <option value="湖南">湖南</option>
                            </c:if>
                            <c:if test="${secuser.address == '湖南'}">
                                <option value="广东" >广东</option>
                                <option value="广西">广西</option>
                    ...
                    <div class="form-group">
                        ...
                        <select name="ucode" class="form-control" id="ucode">
                            <c:if test="${secuser.ucode == '1'}">
                                <option value="1" selected>1</option>
                                <option value="2">2</option>
                                <option value="3">3</option>
                            </c:if>
                            <c:if test="${secuser.ucode == '2'}">
                                <option value="1" >1</option>
                                <option value="2" selected>2</option>
                                <option value="3">3</option>
                            </c:if>
                            <c:if test="${secuser.ucode == '3'}">
                                <option value="1" selected>1</option>
                                <option value="2">2</option>
